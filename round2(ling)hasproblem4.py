...
class Trader:
    ...
    def basket_arbitrage(self, state, traderObject, result, arbitrage_handled_products):
        """Execute basket arbitrage strategy with z-score threshold and position sizing"""
        
        for basket, components in BASKET_COMPOSITIONS.items():
            # Skip if we don't have all required order depths
            if basket not in state.order_depths:
                self.logger.print(f"SKIP: {basket} not in order_depths")
                continue
            ...
            if current_spread is None:
                self.logger.print(f"SKIP: {basket} spread is None")
                continue
            
            # Calculate z-score using spreads history
            spreads_history_key = f"{basket.lower()}_spreads"
            spreads_history = traderObject.get(spreads_history_key, [])
            
            # Need at least a few data points to calculate meaningful z-score
            if len(spreads_history) < 5:
                self.logger.print(f"SKIP: Not enough spread history for {basket} to calculate z-score")
                continue
            
            # Calculate mean and standard deviation of spreads
            mean_spread = sum(spreads_history) / len(spreads_history)
            variance = sum((x - mean_spread) ** 2 for x in spreads_history) / len(spreads_history)
            std_spread = variance ** 0.5  # Standard deviation
            
            # Avoid division by zero
            if std_spread == 0:
                self.logger.print(f"SKIP: Standard deviation is zero for {basket}")
                continue
            
            # Calculate z-score
            z_score = (current_spread - mean_spread) / std_spread
            
            self.logger.print(f"{basket} - Current spread: {current_spread:.2f}, Mean: {mean_spread:.2f}, Std: {std_spread:.2f}, Z-score: {z_score:.2f}")
            
            # Initialize orders for this product
            basket_orders = []
            component_orders = {comp: [] for comp in components}
            
            # IMPORTANT: Track whether we're going to use arbitrage
            will_use_arbitrage = False
            
            # Calculate position proportion based on z-score
            # If |z-score| > 0.3, go all in (95% of position limit)
            # Otherwise, use concave function based on spread magnitude
            abs_z_score = abs(z_score)
            
            if abs_z_score > 0.3:
                # Go all in with 95% of position limits
                proportion = 0.95
                self.logger.print(f"Z-score {z_score:.2f} > 0.3: Going all in with {proportion:.2%} of position limits")
            else:
                # Use concave function based on the spread magnitude
                # (spread/100)^0.7
                abs_spread = abs(current_spread)
                proportion = min(0.8, (abs_spread / 100) ** 0.7)
                self.logger.print(f"Z-score {z_score:.2f} <= 0.3: Using concave function with {proportion:.2%} of position limits")
            
            if z_score > 0:  # Spread is higher than average - Sell basket, buy components
                self.logger.print(f"Attempting SELL basket, BUY components arbitrage - z-score: {z_score:.2f}")
                
                # Determine max trade size for basket (selling)
                basket_limit = self.LIMIT[basket]
                max_sell_basket = int((basket_limit + basket_position) * proportion)
                max_sell_basket = max(1, max_sell_basket)  # Ensure at least 1 unit
                
                # Determine max trade size for each component (buying)
                component_max_trades = {}
                for component, quantity in components.items():
                    component_limit = self.LIMIT[component]
                    current_position = component_positions[component]
                    # Max buy amount considering position limit and proportion
                    max_buy_units = int((component_limit - current_position) / quantity * proportion)
                    component_max_trades[component] = max(1, max_buy_units)  # Ensure at least 1 unit
                
                # Find the limiting factor (basket or component)
                max_basket_trades = max_sell_basket
                for component, max_trades in component_max_trades.items():
                    max_basket_trades = min(max_basket_trades, max_trades)
                
                self.logger.print(f"Max possible basket trades based on position limits: {max_basket_trades}")
                
                # Check order book liquidity
                available_basket_trades = 0
                if len(state.order_depths[basket].buy_orders) > 0:
                    ...
                    available_basket_trades = basket_bid_volume
                    self.logger.print(f"Can sell up to {available_basket_trades} {basket} at {best_basket_bid}")
                else:
                    self.logger.print(f"NO BID PRICE: {basket} has no buy orders")
                    continue  # Skip if no liquidity
                
                # Check component liquidity
                component_available_trades = {}
                ...
                for component, quantity in components.items():
                    if len(state.order_depths[component].sell_orders) > 0:
                        ...
                        max_trades_from_liquidity = int(component_ask_volume / quantity)
                        component_available_trades[component] = (max_trades_from_liquidity, best_component_ask)
                        self.logger.print(f"Can buy up to {max_trades_from_liquidity} units of {basket} worth of {component} at {best_component_ask}")
                    else:
                        self.logger.print(f"NO ASK PRICE: {component} has no sell orders")
                        all_components_have_liquidity = False
                        break
                
                if not all_components_have_liquidity:
                    continue  # Skip if any component lacks liquidity
                
                # Calculate the final trade size (minimum of position limits and available liquidity)
                final_trade_size = max_basket_trades
                for component, (max_trades, _) in component_available_trades.items():
                    final_trade_size = min(final_trade_size, max_trades)
                
                final_trade_size = min(final_trade_size, available_basket_trades)
                
                # Only proceed if we can trade at least 1 unit
                if final_trade_size >= 1:
                    self.logger.print(f"DECISION: Will sell {final_trade_size} {basket}")
                    
                    # Get best price from order book
                    best_basket_bid = max(state.order_depths[basket].buy_orders.keys())
                    basket_orders.append(Order(basket, best_basket_bid, -final_trade_size))
                    
                    # Add component orders
                    for component, quantity in components.items():
                        best_ask = component_available_trades[component][1]
                        buy_quantity = quantity * final_trade_size
                        self.logger.print(f"DECISION: Will buy {buy_quantity} {component} at {best_ask}")
                        component_orders[component].append(Order(component, best_ask, buy_quantity))
                    
                    will_use_arbitrage = True
                else:
                    self.logger.print(f"INSUFFICIENT QUANTITY: Can only trade {final_trade_size} units")
                    
            elif z_score < 0:  # Spread is lower than average - Buy basket, sell components
                self.logger.print(f"Attempting BUY basket, SELL components arbitrage - z-score: {z_score:.2f}")
                
                # Determine max trade size for basket (buying)
                basket_limit = self.LIMIT[basket]
                max_buy_basket = int((basket_limit - basket_position) * proportion)
                max_buy_basket = max(1, max_buy_basket)  # Ensure at least 1 unit
                
                # Determine max trade size for each component (selling)
                component_max_trades = {}
                for component, quantity in components.items():
                    component_limit = self.LIMIT[component]
                    current_position = component_positions[component]
                    # Max sell amount considering position limit and proportion
                    max_sell_units = int((component_limit + current_position) / quantity * proportion)
                    component_max_trades[component] = max(1, max_sell_units)  # Ensure at least 1 unit
                
                # Find the limiting factor (basket or component)
                max_basket_trades = max_buy_basket
                for component, max_trades in component_max_trades.items():
                    max_basket_trades = min(max_basket_trades, max_trades)
                ...
                if len(state.order_depths[basket].sell_orders) > 0:
                    ...
                    available_basket_trades = basket_ask_volume
                    self.logger.print(f"Can buy up to {available_basket_trades} {basket} at {best_basket_ask}")
                else:
                    self.logger.print(f"NO ASK PRICE: {basket} has no sell orders")
                    continue  # Skip if no liquidity
                ...
                for component, quantity in components.items():
                    if len(state.order_depths[component].buy_orders) > 0:
                        ...
                        max_trades_from_liquidity = int(component_bid_volume / quantity)
                        component_available_trades[component] = (max_trades_from_liquidity, best_component_bid)
                        self.logger.print(f"Can sell up to {max_trades_from_liquidity} units of {basket} worth of {component} at {best_component_bid}")
                    else:
                        self.logger.print(f"NO BID PRICE: {component} has no buy orders")
                        all_components_have_liquidity = False
                        break
                ...
                # Only proceed if we can trade at least 1 unit
                if final_trade_size >= 1:
                    self.logger.print(f"DECISION: Will buy {final_trade_size} {basket}")
                    
                    # Get best price from order book
                    best_basket_ask = min(state.order_depths[basket].sell_orders.keys())
                    basket_orders.append(Order(basket, best_basket_ask, final_trade_size))
                    
                    # Add component orders
                    for component, quantity in components.items():
                        best_bid = component_available_trades[component][1]
                        sell_quantity = quantity * final_trade_size
                        self.logger.print(f"DECISION: Will sell {sell_quantity} {component} at {best_bid}")
                        component_orders[component].append(Order(component, best_bid, -sell_quantity))
                ...
                self.logger.print(f"SPREAD IS AT MEAN: Z-score is exactly 0.0")
            
            # Add orders to the result if arbitrage will be used
            if will_use_arbitrage:
                # Add basket order
                if basket not in result:
                    result[basket] = []
                result[basket].extend(basket_orders)
                
                # Mark the basket as handled by arbitrage
                ...
                for component, orders in component_orders.items():
                    ...
                    # Mark the component as handled by arbitrage
                    arbitrage_handled_products.add(component)
                
                self.logger.print(f"SUCCESS: Executed {basket} arbitrage trade with z-score {z_score:.2f}!")
    ...
    def run(self, state: TradingState):
        ...
        self.basket_arbitrage(state, traderObject, result, arbitrage_handled_products)
