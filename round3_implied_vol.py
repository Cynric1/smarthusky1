...
class BlackScholes:
    ...
    def implied_volatility(
        ...
        low_vol = 0.01
        high_vol = 1.0
        min_price = BlackScholes.black_scholes_call(spot, strike, time_to_expiry, low_vol)
        if min_price > call_price:
            print("Call mispriced, too cheap")
            return float('nan')
...
class VolcanicVolatilityStrategy(Strategy):
    ...
    def __init__(self, symbol: str, limit: int) -> None:
        ...
        self.position_limits = {
            Product.VOLCANIC_ROCK: min(limit, 400),  # Increased limit for underlying
            Product.VOLCANIC_ROCK_VOUCHER_9500: 200,  # Increased option limits
    ...
    def _trade_volatility_arbitrage(self, state, rock_price, time_to_expiry, realized_vol, current_net_delta):
        ...
        for voucher_symbol, strike_price in self.vouchers.items():
            ...
            # if strike_price < 10000:
            try:
                iv = BlackScholes.implied_volatility(
                    option_price, rock_price, strike_price, time_to_expiry
                )
                print(f"ITM Implied Volatility: {iv}")
            except Exception as e:
                print(f"Error calculating implied volatility: {e}")
                bs_price_iv0 = BlackScholes.black_scholes_call(rock_price, strike_price, time_to_expiry, 1e-10)
                # long if option is udnerpriced
                if bs_price_iv0 > option_price and buy_capacity >= trade_size:
                    print('time to arbitrage')
                    ...
                    if hedge_size > 0:
                        underlying_trades.append((-hedge_size, "hedge for " + voucher_symbol))

                    options_traded += 1
                continue 
            
            # standard logic for strike >= 10000
            ...
            if abs(vol_diff) > self.vol_diff_threshold:
                if vol_diff > 0:  # Implied vol > realized vol (options overpriced)
                    ...
                    if sell_capacity >= trade_size:
                        ...
                        # Calculate approximate delta (should < 0 as we sell options)
                        option_delta = self._calculate_simple_delta(rock_price, strike_price, time_to_expiry)
                    ...
                    if buy_capacity >= trade_size:
                        ...
                        # Calculate approximate delta (should > 0 as we buy options)
                        option_delta = self._calculate_simple_delta(rock_price, strike_price, time_to_expiry)
                        
                        hedge_size = round(trade_size * option_delta)
                        if hedge_size > 0: # short underlying
                            underlying_trades.append((-hedge_size, "hedge for " + voucher_symbol))
        ...
        # remove contrarian logic
        hedge_target = -round(current_net_delta)

        
        # Limit the target to our position limits
        if hedge_target > rock_limit:
            hedge_target = rock_limit
        elif hedge_target < -rock_limit:
            hedge_target = -rock_limit
            
        adjustment = hedge_target - rock_position
