...
class SquidInkStrategy(MarketMakingStrategy):
    ...
    def __init__(self, symbol: str, limit: int) -> None:
        ...
        self.params = {
            ...
            "ou_mu": 1845,             # OU mean reversion level
...
class ETFArbitrageStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        # if zscore >= 0.5:
        #     # Basket is overpriced compared to components - SELL basket, BUY components
        #     basket_max_sell = self.POSITION_LIMIT[self.symbol][self.symbol] + state.position.get(self.symbol, 0)
        #     basket_best_bid = max(state.order_depths[self.symbol].buy_orders.keys())
        #     for component, quantity in self.components.items():
        #         hedge_qty = quantity * hedge_ratio
        #         component_max_buy = self.POSITION_LIMIT[self.symbol][component] - state.position.get(component, 0)
        #         max_basket_trade = min(basket_max_sell, component_max_buy // hedge_qty)
        #     if max_basket_trade <= 0:
        #         return
        #     self.sell(basket_best_bid, int(max_basket_trade))
        #     for component, quantity in self.components.items():
        #         component_ask_price = min(state.order_depths[component].sell_orders.keys())
        #         hedge_qty = quantity * hedge_ratio * max_basket_trade
        #         print(f"SELLING {component} at {component_ask_price} for {int(hedge_qty)}")
        #         self.component_orders[component].append(
        #             Order(component, component_ask_price, int(hedge_qty))
        #         ) if hedge_qty > 0 else None
        # elif zscore <= -0.5:
        #     # Basket is underpriced compared to components - BUY basket, SELL components
        #     basket_max_buy = self.POSITION_LIMIT[self.symbol][self.symbol] - state.position.get(self.symbol, 0)
        #     basket_best_ask = min(state.order_depths[self.symbol].sell_orders.keys())
        #     for component, quantity in self.components.items():
        #         hedge_qty = quantity * hedge_ratio
        #         component_max_sell = self.POSITION_LIMIT[self.symbol][component] + state.position.get(component, 0)
        #         max_basket_trade = min(basket_max_buy, component_max_sell // hedge_qty)
        #     if max_basket_trade <= 0:
        #         return
        #     self.buy(basket_best_ask, int(max_basket_trade))
        #     for component, quantity in self.components.items():
        #         component_bid_price = max(state.order_depths[component].buy_orders.keys())
        #         hedge_qty = quantity * hedge_ratio * max_basket_trade
        #         print(f"BUYING {component} at {component_bid_price} for {int(hedge_qty)}")
        #         self.component_orders[component].append(
        #             Order(component, component_bid_price, -int(hedge_qty))
        #         ) if hedge_qty > 0 else None
        if abs(zscore) < 0.5:
            # close all positions
            position = state.position.get(self.symbol, 0)
            if position > 0:
                ...
                self.sell(basket_ask_price, position)
            elif position < 0:
                basket_bid_price = max(state.order_depths[self.symbol].buy_orders.keys())
                self.buy(basket_bid_price, -position)
            for component, quantity in self.components.items():
                component_position = state.position.get(component, 0)
                if component_position > 0:
                    component_bid_price = max(state.order_depths[component].buy_orders.keys())
                    self.component_orders[component].append(
                        Order(component, component_bid_price, -component_position)
                    )
                elif component_position < 0:
                    component_ask_price = min(state.order_depths[component].sell_orders.keys())
                    self.component_orders[component].append(
                        Order(component, component_ask_price, -component_position)
                    )
        if abs(zscore) >= 1:
            if zscore > 0:
                # Basket is overpriced compared to components - SELL basket, BUY components
                basket_max_sell = self.POSITION_LIMIT[self.symbol][self.symbol] + state.position.get(self.symbol, 0)
                basket_best_bid = max(state.order_depths[self.symbol].buy_orders.keys())
                max_basket_trade = basket_max_sell
                for component, quantity in self.components.items():
                    hedge_qty = quantity * hedge_ratio
                    component_max_buy = self.POSITION_LIMIT[self.symbol][component] - state.position.get(component, 0)
                    max_basket_trade = min(max_basket_trade, component_max_buy // hedge_qty, component_max_buy)
                if max_basket_trade <= 0:
                    return
                self.sell(basket_best_bid, int(max_basket_trade))
                for component, quantity in self.components.items():
                ...
                basket_max_buy = self.POSITION_LIMIT[self.symbol][self.symbol] - state.position.get(self.symbol, 0)
                basket_best_ask = min(state.order_depths[self.symbol].sell_orders.keys())
                max_basket_trade = basket_max_buy
                for component, quantity in self.components.items():
                    hedge_qty = quantity * hedge_ratio
                    component_max_sell = self.POSITION_LIMIT[self.symbol][component] + state.position.get(component, 0)
                    max_basket_trade = min(max_basket_trade, component_max_sell // hedge_qty, component_max_sell)
                if max_basket_trade <= 0:
                    return
                self.buy(basket_best_ask, int(max_basket_trade))
                for component, quantity in self.components.items():
    ...
    def save(self):
        ...
        data = {'basket_price_history': self.basket_price_history,
