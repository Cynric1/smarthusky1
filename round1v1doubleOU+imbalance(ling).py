...
PARAMS = {
    Product.RAINFOREST_RESIN: {
        ...
        "imbalance_weight": 0.06, # Weight of imbalance factor for fair value adjustment
    },
    Product.KELP: {
        ...
        "ou_theta": 0.4,        # OU mean reversion speed
        "ou_mu": 2035.0895,          # OU mean reversion level for kelp
        "ou_sigma": 45,         # OU volatility
        "momentum_lookback": 3, # Momentum lookback window
        "momentum_weight": 0.4, # Weight of momentum component (OU weight = 1 - momentum_weight)
        "disregard_edge": 1,
        "join_edge": 1,
        "default_edge": 2,     # Tighter spread for more active trading
        "soft_position_limit": 40,
        "imbalance_weight": 0.02, # Weight of imbalance factor for fair value adjustment
    },
    Product.SQUID_INK: {
        ...
        "ou_mu": 1837.6465,     # OU mean reversion level
        ...
        "default_edge": 2,     # Tighter spread for more active trading
        "soft_position_limit": 40,
        "imbalance_weight": 0.035, # Weight of imbalance factor for fair value adjustment
    },
}
...
class Trader:
    ...
    def calculate_order_book_imbalance(self, order_depth: OrderDepth, depth_levels: int = 5) -> float:
        """
        Calculate the order book imbalance factor
        Returns a value between -1 (sell heavy) and 1 (buy heavy)
        """
        if not order_depth.buy_orders or not order_depth.sell_orders:
            return 0.0  # No imbalance if one side is empty
            
        # Get total buy and sell volume (considering up to depth_levels levels)
        buy_prices = sorted(list(order_depth.buy_orders.keys()), reverse=True)[:depth_levels]
        sell_prices = sorted(list(order_depth.sell_orders.keys()))[:depth_levels]
        
        buy_volume = sum(order_depth.buy_orders[price] for price in buy_prices)
        sell_volume = sum(abs(order_depth.sell_orders[price]) for price in sell_prices)
        ...
        # Apply volume-weighted price pressure component
        # This gives more weight to orders closer to the mid price
        weighted_buy_pressure = 0
        weighted_sell_pressure = 0
        
        if buy_prices and sell_prices:
            mid_price = (buy_prices[0] + sell_prices[0]) / 2
            
            for price in buy_prices:
                distance = 1 / max(1, abs(mid_price - price))  # Closer = higher weight
                weighted_buy_pressure += order_depth.buy_orders[price] * distance
                
            for price in sell_prices:
                distance = 1 / max(1, abs(price - mid_price))  # Closer = higher weight
                weighted_sell_pressure += abs(order_depth.sell_orders[price]) * distance
            
            total_pressure = weighted_buy_pressure + weighted_sell_pressure
            if total_pressure > 0:
                weighted_imbalance = (weighted_buy_pressure - weighted_sell_pressure) / total_pressure
                # Combine simple imbalance with weighted imbalance
                imbalance = (imbalance + weighted_imbalance) / 2
    ...
    def calculate_fair_value(self, product: str, order_depth: OrderDepth, traderObject) -> float:
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            # Apply imbalance factor to fair value calculation
            imbalance_adjustment = imbalance * self.params[product]["imbalance_weight"] * ou_sigma
            ...
            fair += imbalance_adjustment
            
            # Log some debugging info
            self.logger.print(
                f"{product} - Mid: {mid_price:.2f}, OU: {ou_prediction:.2f}, "
                f"Momentum: {momentum_prediction:.2f}, Imbalance: {imbalance:.2f}, "
                f"Adjustment: {imbalance_adjustment:.2f}, Fair: {fair:.2f}"
            )
    ...
    def adjust_fair_value_with_imbalance(self, product: str, fair_value: float, order_depth: OrderDepth) -> float:
        """Adjust fair value based on order book imbalance"""
        # Calculate imbalance factor
        imbalance = self.calculate_order_book_imbalance(order_depth)
        
        # Apply imbalance adjustment - higher imbalance means stronger price movement expectation
        # Scale by imbalance_weight parameter
        imbalance_weight = self.params[product].get("imbalance_weight", 0.2)
        
        # For Rainforest Resin, we'll use a fixed volatility estimate
        if product == Product.RAINFOREST_RESIN:
            volatility_estimate = 5  # Fixed volatility estimate for stable product
        else:
            # For other products, use the OU sigma as volatility estimate
            volatility_estimate = self.params[product].get("ou_sigma", 20)
        
        # Calculate adjustment amount based on imbalance and volatility
        adjustment = imbalance * imbalance_weight * volatility_estimate
        
        # Apply adjustment to fair value
        adjusted_fair_value = fair_value + adjustment
        
        # Log the adjustment
        self.logger.print(
            f"{product} Imbalance: {imbalance:.4f}, Adjustment: {adjustment:.2f}, "
            f"Original Fair: {fair_value:.2f}, Adjusted Fair: {adjusted_fair_value:.2f}"
        )
    ...
    def make_orders(
        ...
        # Calculate order book imbalance to adjust our bid-ask spread
        imbalance = self.calculate_order_book_imbalance(order_depth)
        
        # Adjust spread based on imbalance - tighten spread when imbalance is strong
        # This allows us to be more aggressive when there's clear directional movement
        imbalance_factor = 1.0 - abs(imbalance) * 0.4  # 0.6 to 1.0 multiplier on spread
        adjusted_edge = max(1, round(default_edge * imbalance_factor))
        
        # Set ask price based on existing orders
        ask = round(fair_value + adjusted_edge)
        ...
        bid = round(fair_value - adjusted_edge)
        ...
        # Further adjust ask/bid based on strong imbalance
        if imbalance > 0.5:  # Strong buying pressure, prices likely to rise
            ask += 1  # Raise ask price
            bid += 1  # Raise bid price
        elif imbalance < -0.5:  # Strong selling pressure, prices likely to fall
            ask -= 1  # Lower ask price
            bid -= 1  # Lower bid price
    ...
    def run(self, state: TradingState):
        ...
        if Product.RAINFOREST_RESIN in state.order_depths:
            ...
            # Calculate initial fair value for Resin
            resin_fair_value = self.params[Product.RAINFOREST_RESIN]["fair_value"]
            
            # Adjust fair value based on order book imbalance
            resin_fair_value = self.adjust_fair_value_with_imbalance(
                Product.RAINFOREST_RESIN,
                resin_fair_value,
