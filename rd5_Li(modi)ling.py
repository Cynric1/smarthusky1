...
class KelpStrategy(MarketMakingStrategy):
    def __init__(self, symbol: str, limit: int) -> None:
        super().__init__(symbol, limit)
        # Parameters for informed trader detection
        self.adverse_volume_threshold = 28  # Consider orders larger than this as potentially informed
        self.price_history = []  # Track price history to detect trends
        self.max_history_size = 10
        self.reversion_beta = -0.2  # Mean reversion parameter (negative like in the first code)
        self.last_fair_value = None
        
    def get_true_value(self, state: TradingState) -> int:
        order_depth = state.order_depths[self.symbol]
        
        # If no orders, use fallback
        if not order_depth.buy_orders or not order_depth.sell_orders:
            return 2026  # Fallback value
        
        # Calculate regular midpoint price
        best_bid = max(order_depth.buy_orders.keys())
        best_ask = min(order_depth.sell_orders.keys())
        mid_price = (best_bid + best_ask) / 2
        
        # Filter out potentially orders from informed traders (large orders)
        filtered_ask_prices = [
        ...
        # Get the most popular prices (highest volume)
        ...
        # If we have filtered prices from larger (likely market maker) orders, use them
...
class SquidInkStrategy(MarketMakingStrategy):
    ...
    def __init__(self, symbol: str, limit: int) -> None:
        ...
        self.params = {
            ...
            "ou_mu": 2000,             # OU mean reversion level
...
class VolcanicVolatilityStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        print(f"PORTFOLIO DELTA: {net_delta}")
    ...
    def _trade_volatility_arbitrage(self, state, rock_price, current_net_delta):
        ...
        for voucher_symbol, strike_price in self.vouchers.items():
            ...
            if abs(vol_diff) > self.IV_std[strike_price]:
                ...
                if current_position < 0:
                    ...
                    self._add_order(voucher_symbol, price, buy_capacity)
            # Full swing
            # if vol_diff > self.IV_std[strike_price]:
            #     # Sell option
            #     price = max(order_depth.buy_orders.keys())
            #     self._add_order(voucher_symbol, price, -sell_capacity)
            #     new_delta -= option_delta * sell_capacity
        ...
        HEDGE = True
        ts = state.timestamp
        if HEDGE and ts % 5000 == 0 and ts != 0:
            delta_after_trade = current_net_delta + new_delta
            underlying_position = state.position.get(self.symbol, 0)
