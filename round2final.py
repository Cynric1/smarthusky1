...
class SquidInkStrategy(MarketMakingStrategy):
    ...
    def __init__(self, symbol: str, limit: int) -> None:
        ...
        self.params = {
            ...
            "ou_mu": 1900,             # OU mean reversion level
...
class PicnicBasketStrategy(Strategy):
    def __init__(self, symbol: str, limit: int, trader_data_key: str = None) -> None:
        ...
        if symbol == Product.PICNIC_BASKET1:
            self.long_threshold = -10
            self.short_threshold = 180
        elif symbol == Product.PICNIC_BASKET2:
            self.long_threshold = -10
            self.short_threshold = 150
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for basket in [Product.PICNIC_BASKET1, Product.PICNIC_BASKET2]:
            ...
            if basket_orders:
                ...
                # Add component orders if we have them
        ...
        for product, strategy in self.strategies.items():
            # Skip products already handled by basket arbitrage
            if product in handled_products:
                self.logger.print(f"Skipping {product} - already handled by basket arbitrage")
                continue
            ...
            # Load previous state if available
            if product in old_trader_data and product not in [Product.PICNIC_BASKET1, Product.PICNIC_BASKET2]:
        ...
        hedge_orders = self.hedging_strategy.execute_hedge(state)
